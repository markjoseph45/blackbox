...
<div class="col-sm-7 container-fluid">
	<div class="panel panel-default">
		<div class="panel-header" id="fashionHeader">
			<h3 class="panel-title">View Fashion</h3>
		</div>
		...
		<div class="row" id="pagerRow" align="center">
			<ul class="pagination" id="ulPager">
			    <li id="previousPager" <?php if ($activePage == 1 || $activePage == 0): ?> class="disabled" <?php endif; ?>>
			      <a <?php if ($activePage != 1 && $activePage != 0): ?> onclick="previous($(this))" <?php endif; ?> id="<?php echo $activePage; ?>" aria-label="Previous" data-category="<?= $category; ?>">
			        <span aria-hidden="true">&laquo;</span>
			      </a>
			    </li>
			    <?php $offset = 0; for ($i=0; $i < $countRow; $i++) { $x = $i + 1; ?>
			    <li id="pageNum">
			    	<a <?php if ($x == $activePage): ?> style="background-color:#f44336;color:white" <?php endif; ?> id="page<?php echo $x; ?>" class="<?php echo $offset;  ?>" href="<?= base_url(); ?>shop/fashion_view/<?= $category; ?>/<?php echo $offset;  ?>/<?php echo $x;  ?>/9"><?php echo $x;  ?></a>
			    </li>
			    <?php $offset = $offset + 9;  } //end of for_loop ?>
				<li id="nextPager" <?php if ($activePage == $x || $activePage == 0): ?> class="disabled" <?php endif; ?>>
			      <a <?php if ($activePage != $x && $activePage != 0): ?> onclick="next($(this))" <?php endif; ?> id="<?php echo $activePage; ?>" aria-label="Next" data-category="<?= $category; ?>">
			        <span aria-hidden="true">&raquo;</span>
			      </a>
			    </li>
			    &nbsp; &nbsp; &nbsp; &nbsp;

			      <a <?php if ($activePage == 0): ?> style="background-color:#f44336;color:white" <?php endif; ?> id="viewAll" class="pull-right" href="<?= base_url(); ?>shop/fashion_view/<?= $category; ?>/0/0/0">
...
function previous($scope){
	...
	var offset = off - 9;
	var x = pagerId - 1;
	var href = base+'shop/fashion_view/'+category+'/'+offset+'/'+x+'/9';
